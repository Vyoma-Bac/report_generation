...
async def fetch_and_process_data(userId, startTime, endTime):
    sample_rate = 200
    chunk_size = 7200 * 1000  # 1800 seconds in milliseconds (30 minutes)
    current_time = int(startTime)
    tasks = []
    endTime = int(endTime)
    # Create a thread pool executor for processing chunks
    with ThreadPoolExecutor(max_workers=40) as executor:
        ...
        while current_time < endTime:
            ...
            # Fetch data chunk
            data_chunk = await fetch_data_chunk(userId, current_time, chunk_end_time)
            
            # Process data chunk
            futures = [loop.run_in_executor(executor, process_ecg_data_chunk, chunk, sample_rate) for chunk in data_chunk]
            processed_chunks = await asyncio.gather(*futures)
            # Append processed chunks to results
            tasks.extend(processed_chunks)
...
async def generate_pdf(userId, startDate, endDate):
  ...
  json_data = await fetch_and_process_data(userId, startDate, endDate)
